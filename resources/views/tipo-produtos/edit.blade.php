<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            <h1>
                {{ __('Alterar Tipo Produto') }}
            </h1>
        </div>
    </x-slot>
    <div class="py-4">
        <form action="{{ route('tipo-produtos.update', $tipoProduto->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div class="md:col-span-1">
                            ...
                            <div class="flex items-center gap-3 p-[6px] border border-gray-300 rounded-md shadow-sm">
                                ...
                                <x-text-input id="status" name="status" type="checkbox" value="1"
                                    ...
                                    :checked="old('status', $tipoProduto->status) == 1" 
                                />
                                <span>Ativo</span>
                            </div>
                        </div>
                        <div class="md:col-span-2">
                            ...
                            <x-text-input id="nome" name="nome" type="text" class="w-full"
                                :value="old('nome', $tipoProduto->nome)" />
                        </div>
                    </div>
                </div>
            </div>

            <x-form-button :cancelUrl="route('tipo-produtos.index')" submitText="Salvar"/>
